<script lang="ts">
    ...
    async function handleSearch(event: Event) {
        ...
        if (error.length !== 0) {
            ...
            notificationData.update(() => 'Failed to search for recipes. Please try again later.');
...
<section in:fly={{ y: -100, duration: 500, delay: 500 }} out:fly={{ duration: 500 }}>
    ...
    {#if $recipeSearchTerm}
    <p>Search results for "{$recipeSearchTerm}": {$recipeListData.length} recipes found</p>
    {/if}
    ...
    <article>
        ...
        <button on:click={() => alert(`You clicked on ${recipe.title}`)}>View Recipe</button>
    </article>
    {/each}
    {:else}
    <p>No recipes</p>
